...
const PERSONALITIES: {
...
}[] = [
  {
    ...
    label: "Professional",
    icon: "💼",
    desc: "Concise, precise, no slang",
  },
  {
    value: "friendly",
    label: "Friendly",
    icon: "😊",
    desc: "Warm, clear, encouraging",
  },
...
const PROVIDERS: {
...
}[] = [
  {
    ...
    label: "Claude",
    icon: "◈",
    desc: "Excellent reasoning for complex decisions",
  },
  {
    value: "openai",
    label: "GPT-4o",
    icon: "◉",
    desc: "Fast and versatile all-around",
  },
  {
    value: "gemini",
    label: "Gemini",
    icon: "◇",
    desc: "Quick responses and solid performance",
  },
];
...
const ALL_ACTIONS: {
  ...
  { key: "swap", label: "Swap", icon: "⇄", desc: "Swap via Jupiter DEX" },
  {
    ...
    label: "Stake",
    icon: "⬡",
    desc: "Stake SOL with validators",
  },
];
    ...
    <View className="flex-1 items-center justify-center px-6 gap-8">
      ...
      <View className="items-center gap-3">
        ...
        <Text className="text-text-secondary text-base text-center leading-6 max-w-xs">
          Your Solana wallet companion. Ask for advice or execute actions: check
          balances, swap tokens, send SOL, stake — all from a chat.
        </Text>
      </View>
      <View className="w-full gap-3">
        {[
          "Ask for portfolio advice",
          "Execute swaps and sends",
          "You approve high-value moves",
          "Honest about limitations",
        ].map((f) => (
          <View
            key={f}
            className="flex-row items-center gap-3 bg-card border border-border rounded-2xl px-4 py-3"
          >
...
function StepIdentity({
  ...
  setPersonality: (v: AgentPersonality) => void;
  onBack: () => void;
  onNext: () => void;
}) {
  return (
        ...
        {PERSONALITIES.map((p) => (
          ...
          >
            ...
            <View className="flex-1">
              <Text
                className={`font-bold text-base ${personality === p.value ? "text-accent" : "text-text-primary"}`}
              >
                {p.label}
              </Text>
  ...
  return (
    ...
    >
      <View className="gap-1">
        ...
        <Text className="text-text-secondary text-sm">
          Pick the language model powering your agent
        </Text>
      </View>
      ...
      <View className="gap-3">
        {PROVIDERS.map((p) => (
          ...
          >
            ...
            >
              <Text
                className={`text-2xl font-bold ${provider === p.value ? "text-accent" : "text-text-muted"}`}
              >
                {p.icon}
              </Text>
            </View>
            <View className="flex-1">
              <Text
                className={`font-bold text-base ${provider === p.value ? "text-accent" : "text-text-primary"}`}
              >
                {p.label}
              </Text>
              <Text className="text-text-muted text-xs mt-0.5">{p.desc}</Text>
      ...
      <View className="bg-card border border-border rounded-2xl p-4 gap-1">
        ...
        <Text className="text-text-secondary text-xs leading-4">
          API keys are stored securely in Supabase Edge Function secrets — never
    ...
    >
      ...
      <View className="gap-3">
        ...
        <Text className="text-text-muted text-xs leading-4">
          Actions below this value execute autonomously. At or above — you'll be
      ...
      <View className="gap-3">
        ...
        {ALL_ACTIONS.map((a) => (
          ...
          >
            ...
            >
              <Text
                className={`text-lg font-bold ${actions.includes(a.key) ? "text-accent" : "text-text-muted"}`}
              >
                {a.icon}
              </Text>
            </View>
...
async function ensureSupabaseSession(privyUser: any): Promise<string | null> {
  ...
  const email: string | undefined = accounts.find(
    (a: any) => a.type === "email",
  )?.address;
...
export default function AgentSetupScreen() {
  ...
  const [personality, setPersonality] =
    useState<AgentPersonality>("professional");
  ...
  return (
    <SafeAreaView className="flex-1 bg-bg" edges={["top"]}>
      ...
      <View className="flex-1">
        {step === 1 && <StepIntro onNext={() => animateStep(2)} />}
        {step === 2 && (
          <StepIdentity
            agentName={agentName}
